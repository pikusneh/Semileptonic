...
std::map<std::string, std::vector<std::string>> readInputFiles(const std::string &filename) {
    ...
    while (std::getline(infile, line)) {
        if (line.empty() || line[0] == '#') {  // Handle comments and empty lines
            if (!line.empty() && line[0] == '#') {
                currentDataset = line.substr(1);  // Get dataset name without '#'
                datasets[currentDataset] = std::vector<std::string>();
            }
            continue;
        }
        if (!currentDataset.empty()) {
            datasets[currentDataset].push_back(line);
        }
    }
    return datasets;
}

void myskim(bool isMC, const std::string &year, const std::string &filelist) {
    auto datasets = readInputFiles(filelist);

    for (const auto &dataset : datasets) {
        const std::string &datasetName = dataset.first;
        const std::vector<std::string> &inputFiles = dataset.second;
        ...
        for (const auto &branchName : branchList) {
            if (startsWith(branchName.c_str(), "Electron_") ||
                ...
                startsWith(branchName.c_str(), "HLT_") ||
                startsWith(branchName.c_str(), "L1")) ||
                srartsWith(branchName.c_str(), "MET_") ||
        ...
        auto df1 = df.Define("Pass_trigger_mu", [year](bool HLT_IsoMu24, bool HLT_IsoTkMu24, bool HLT_Mu50, bool HLT_TkMu50, bool HLT_Mu45_eta2p1) {
            if (year == "2016") {
                return HLT_IsoMu24 || HLT_IsoTkMu24 || HLT_Mu50 || HLT_TkMu50 || HLT_Mu45_eta2p1;
            }
            return false;
        }, {"HLT_IsoMu24", "HLT_IsoTkMu24", "HLT_Mu50", "HLT_TkMu50", "HLT_Mu45_eta2p1"})

        .Define("Pass_trigger_ele", [year](bool HLT_Ele27_WPTight_Gsf, bool HLT_Ele105_CaloIdVT_GsfTrkIdT, bool HLT_Ele115_CaloIdVT_GsfTrkIdT, bool HLT_Photon175) {
            if (year == "2016") {
        ...
        .Define("filters", [year](bool Flag_goodVertices, bool Flag_globalSuperTightHalo2016Filter, bool Flag_HBHENoiseFilter, bool Flag_HBHENoiseIsoFilter, bool Flag_EcalDeadCellTriggerPrimitiveFilter, bool Flag_BadPFMuonFilter) {
            ...
            return basicFilters;
        }, {"Flag_goodVertices", "Flag_globalSuperTightHalo2016Filter", "Flag_HBHENoiseFilter", "Flag_HBHENoiseIsoFilter", "Flag_EcalDeadCellTriggerPrimitiveFilter", "Flag_BadPFMuonFilter"})

        .Filter([](bool Pass_trigger_mu, bool Pass_trigger_ele, bool filters) {
            return (Pass_trigger_ele || Pass_trigger_mu) && filters;
        }, {"Pass_trigger_mu", "Pass_trigger_ele", "filters"});

        std::string outputFileName ="/eos/user/s/ssnehshu/"+ datasetName + "_skimmed.root";
        df1.Snapshot("Events", outputFileName, branches);
        
    }
}

void skim() {
    myskim(true, "2016", "filelist.txt");
}
